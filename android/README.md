...
当前版本 **1.7.0**（versionCode 8），包名 com.medtime.app。最低 Android 8.0（API 26），目标 Android 15（API 35），无原生 CPU 架构限制。前端来自相邻 web 目录，构建时加入 APK 的 assets/www。

本版新增空列表首次引导、规格与剂型、药品独立时间、睡前时段、疗程结束日期、暂停／归档、10 分钟稍后提醒和锁屏试响。发布包沿用原签名，可覆盖此前版本并保留私有存储。
...
准备脚本从 Microsoft 和 Google 官方站点下载固定版本的便携 JDK 17、Android Platform 35、Build Tools 35，校验 SHA-256，不改变系统环境变量。默认工具链、中间产物和私有签名位于项目的 work/android-build。构建脚本无需网络，输出 ../medtime-1.7.0.apk 和校验文件。
...
导入按稳定 ID 合并，不覆盖本地已设置的信息、状态和已有记录。药品备份不含设备闹钟开关、系统授权、原生延后队列或试响结果。
...
10 分钟延后使用独立的持久化队列与 PendingIntent，以原时段及原触发时间区分。仅允许同日仍有效且未完成的安排；完成、改时、暂停、归档或疗程结束后取消。延后和停止不写用药记录。正式提醒优先于试响。

AlarmSoundService 是有通知的前台音频服务，正常最多响铃 60 秒、试响 5 秒，播放系统闹钟铃声并振动。通知提供停止、稍后 10 分钟和打开应用。权限就绪或播放器启动不等于实际听到，需用户确认。
...
| getAlarmStatus() | 权限、音量、后台限制、排程、延后、响铃与试响状态 |
...
本版 57 项 Node 测试、61 项纯 Java 断言、28 组浏览器检查通过。覆盖迁移、独立时间、结束日期、暂停归档、记录快照、备份合并、去重、延后资格、闰日、夏令时和时区。浏览器在 320／360／432／1280 像素宽度检查首次设置、记录编辑／删除／撤销、规格区分、搜索、导入导出和旧数据迁移。
