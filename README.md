...
当前版本 **1.8.0**。新安装不预置药品、用量、频率或演示记录。
...
[下载 Android APK（1.8.0）](./medtime-1.8.0.apk?raw=true) · [SHA-256](./medtime-1.8.0.apk.sha256) · [使用说明](./使用说明.md) · [更新记录](./CHANGELOG.md)
...
验证结果：**70 项 Node 测试、61 项纯 Java 断言、35 组浏览器检查**。Edge／Playwright 覆盖 320／360／432／1280 宽度、200% 系统字号及部分 250% 弹窗场景；检查内容、错误提示层、控制台、触摸滑动、焦点、输入标签、备份成功／失败／取消、加密解密和重复导入。Android 接口在浏览器中明确模拟。
...
无网络、联系人、位置、相机或广泛存储权限；无需账号，没有广告和统计 SDK。外部反馈只在用户点击时交给系统浏览器，不自动附带用药数据或发送信息。
